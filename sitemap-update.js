...
const filterUniqueURLs = () => {
    fs.readFile('sitemap.xml', (err, data) => {
        if (data) {
            ...
            // Ensure that urlset and url are arrays
            if (!existingSitemapList.urlset || !Array.isArray(existingSitemapList.urlset.url)) {
                existingSitemapList.urlset = { url: [] };
            }

            let existingSitemapURLStringList = existingSitemapList.urlset.url.map(ele => ele.loc._text);
