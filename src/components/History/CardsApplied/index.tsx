...
const CardsApplied: React.FC<ICardsApplied> = (props) => {
  ...
  const getAllApplied = async () => {
    try {
      const result = await historyApplicator.getAllSubmitedApplied(0, count)
  ...
  const handleClickAddItem = async () => {
    ...
    setCount(newCount)
    try {
      const result = await historyApplicator.getAllSubmitedApplied(0, newCount)
      console.log('result', result)
      if (result) {
        setDataApplied(result.data)
      }
  ...
  return (
    <>
      ...
      >
        ...
        >
          {dataApplied?.length} công việc đã ứng tuyển {activeChild}
        </Typography>
        <TextField
          select
          id="sex"
          ...
          <Skeleton loading={loading} active>
            {dataApplied?.map((posted: any, i: number) => (
              <Card
                ...
                onClick={(e) => handleClickCard(e, posted.id)}
                key={i}
              >
