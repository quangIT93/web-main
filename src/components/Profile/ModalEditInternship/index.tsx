...
interface IModalInternship {
    openModalEditInternship: boolean;
    setOpenModalEditInternship: React.Dispatch<React.SetStateAction<boolean>>;
    setInternshipValues: React.Dispatch<React.SetStateAction<any>>;
    internshipId: any;
    internshipValues: any;
}
const ModalEditInternship: React.FC<IModalInternship> = (props) => {
    const languageRedux = useSelector((state: RootState) => state.changeLaguage.language);
    const language = useSelector((state: RootState) => state.dataLanguage.languages);
    const { openModalEditInternship, setOpenModalEditInternship, setInternshipValues, internshipId, internshipValues } =
        props;
    ...
    const handleGetInternship = async () => {
        if (internshipId !== null && internshipValues.length > 0) {
            setIntership(internshipValues.at(internshipId))
        }
    }
    ...
    const handleClose = () => setOpenModalEditInternship(false);
    ...
    const handleSubmit = () => {
        internshipValues.splice(internshipId, 1, internship)
        setInternshipValues(internshipValues)
        setOpenModalEditInternship(false)
    }

    return (
        <Modal
            open={openModalEditInternship}
            onClose={handleClose}
