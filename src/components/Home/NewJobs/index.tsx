...
const NewJobs: React.FC = () => {
  ...
  const handleChange = async (event: React.ChangeEvent<unknown>, value: number) => {
    ...
    const categoryId = searchParams.get(`categories-id`) ? searchParams.get(`categories-id`) : null
    const thersholdId = postNewest.data.posts[postNewest.data.posts.length - 1].id

    const result = await postApi.getPostNewest(Number(categoryId), null, null, 9, thersholdId)
    ...
    <>
      {
        ...
        <Box sx={{ flexGrow: 1 }} ref={listRef} >
                ...
                >
                  <ImageListItem key={item.image} sx={{ flex: 1, display: 'flex' }}>
                    <img
                      ...
                      alt={item.title}
                      loading="lazy"
                      style={{ width: '120px', maxWidth: 'auto', height: '100%', maxHeight: 130, borderRadius: 10 }}
                    />
                    <div style={{ padding: '0', marginLeft: '12px' }} className='div-cart-item-post'>
                      <Typography
                        gutterBottom
                        variant="h6"
                        component="div"
          ...
          >
            ...
            <Space className='div-hover-more' onClick={(e) => { handleChange(e, page) }}>
