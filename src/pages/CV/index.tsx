    ...
    const printDocument = async (isDowload: boolean) => {
        ...
        for (const divtoprint of divtoprints) {
            ...
            await html2canvas(divtoprint as HTMLElement).then(canvas => {
                ...
                pdf.addImage(imgData, 'PNG', 0, 0, Math.round(canvas.width * ratio), Math.round(canvas.height * ratio));
                pdf.addPage();
    ...
    return (
        <div className="cv-container">
            ...
            <div className="cv-content">
                ...
                <Button onClick={() => { printDocument(true) }} type="primary" shape="round" icon={<DownloadOutlined />}>
                    Download
                </Button>
                <Button onClick={() => { printDocument(false) }} type="primary" shape="round" icon={<DownloadOutlined />}>
                    Preview
                </Button>
            </div>
            <Footer />
