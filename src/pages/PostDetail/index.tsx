...
import {
  CopyLinkPostDetailIcon,
  FaceSharePostDetailIcon,
  SavePostDetailIcon,
  TiktokPostDetailIcon,
  YoutubePostDetailIcon,
} from './post_detail_icon';
// const itemsShare = [
//   {
  ...
  const handleChangeStatus = async () => {
    try {
      ...
      if (result && post?.data?.applied) {
        ...
        let updateCountAppliedJobs = '';
        if (localStorage.getItem('updateCountAppliedJobs')) {
          localStorage.getItem('updateCountAppliedJobs') === 'true'
            ? (updateCountAppliedJobs = 'false')
            : (updateCountAppliedJobs = 'true');
        } else {
          updateCountAppliedJobs = (!updateDefaultCountAppliedJobs).toString();
        }
        localStorage.setItem('updateCountAppliedJobs', updateCountAppliedJobs);
      }
    } catch (error) {
      setTextButton(
        languageRedux === 1
                    ...
                    {post?.data.postCategories.map(
                      (item: any, index: number) => (
                        <Button key={index} className="btn" type="text">
                          {item.parentCategory.fullName.replace('/', ' • ')}
                          {' / '}
                          {item.fullName}
                        </Button>
                      ),
                  ...
                  <div className="description">
                    ...
                    <div className="post_actions_wrapper">
                      <Space
                        ...
                        wrap={true}
                      >
                        <Space className="media-wrapper" size={12} wrap={true}>
                          <Tooltip
                            title={
                              languageRedux === 1
                        ...
                        <Space className="share-save-wrapper" size={12} wrap={true}>
                          <Tooltip
                            title={
                              languageRedux === 1
